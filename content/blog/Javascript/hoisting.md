...
## 호이스팅은 어떻게 일어나는 것인가
호이스팅은 w3C 명세에 표준으로 기술되어있지 않다. 물리적으로 변수의 '선언'부분이 이동하는건 아니지만, 그렇게 동작하긴 한다. 조금더 자세히 말하자면, 함수가 호출되어 실행컨텍스트가 생성되어 변수객체 자료구조에 식별자 정보가 담긴다. <del>사실 개인적 사견으로는, 자바스크립트가 컴파일 되는 과정에 렉싱(토큰화), 파싱(AST 트리 형성)과정을 통해 이미 변수정보를 모두 알고 있기 때문에 실행컨텍스트를 만들수 있는것이고, 엄밀히 말하면 컴파일되는 과정에서 이미 호이스팅이 발생하지 않았나 싶다.</del> 이때 ECMA 문서 스펙에 따라, TDZ의 영향을 받는지의 차이에 따라 `Reference Error` 또는 `undefined` 결과를 출력할 뿐이다.
